...
function App() {
  ...
  const fetchFiles = async () => {
    try {
      // Prevent browser caching which can cause the UI to show "No documents" when files exist
      const res = await fetch(`${API_BASE}/list-files`, { cache: 'no-store' });
                  ...
                  style={{
                    width: `${
                      stats.token_budget.api_calls_made > 0
                        ? (stats.token_budget.model_20b_calls / stats.token_budget.api_calls_made) * 100
                        : 50
                    }%`,
                  }}
                  title={`GPT-OSS 20B: ${stats.token_budget.model_20b_calls} calls`}
                ></div>
                <div
                  className="bar-120b"
                  style={{
                    width: `${
                      stats.token_budget.api_calls_made > 0
                        ? (stats.token_budget.model_120b_calls / stats.token_budget.api_calls_made) * 100
                        : 50
                    }%`,
                  }}
                  title={`GPT-OSS 120B: ${stats.token_budget.model_120b_calls} calls`}
                ></div>
              </div>
              <div className="router-legend">
                <span className="legend-20b">20B ({stats.token_budget.model_20b_calls})</span>
                <span className="legend-120b">120B ({stats.token_budget.model_120b_calls})</span>
              </div>
            </div>
          </div>
        </section>
